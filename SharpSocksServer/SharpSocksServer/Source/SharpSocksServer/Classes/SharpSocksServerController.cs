...
namespace SharpSocksServer.SharpSocksServer.Classes
{
    ...
    public class SharpSocksServerController
    {
        ...
        public List<PSObject> Status
        {
            get
            {
               return SocksProxy.ConnectionDetails.Select(x => new PSObject(x)).ToList();
            }
        }
        ...
        void AcceptTcpClient(IAsyncResult iar)
        {
            ...
            //TO Do: Better error handling around here!!!
            TcpClient tc = null;
            try
            {
            ...
            Task.Factory.StartNew(() =>
            {
                try
                {
                    ...
                    (new SocksProxy()).ProcessRequest(tc, WaitOnConnect);
                }
                catch (Exception ex)
                {
